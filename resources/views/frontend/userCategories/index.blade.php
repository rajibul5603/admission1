        ...
        <div class="col-md-12">
            @can('user_category_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.user-categories.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $userCategory->id }}">
                                        ...
                                        <td>
                                            @can('user_category_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.user-categories.show', $userCategory->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('user_category_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.user-categories.edit', $userCategory->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('user_category_delete')
                                                <form action="{{ route('frontend.user-categories.destroy', $userCategory->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('user_category_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.user-categories.massDestroy') }}",
