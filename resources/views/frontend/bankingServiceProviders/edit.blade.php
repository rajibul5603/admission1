        ...
        <div class="col-md-12">
            ...
            <div class="card">
                <div class="card-header">
                    {{ trans('global.edit') }} {{ trans('cruds.bankingServiceProvider.title_singular') }}
                </div>

                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.banking-service-providers.update", [$bankingServiceProvider->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group">
                            <label class="required" for="name">{{ trans('cruds.bankingServiceProvider.fields.name') }}</label>
                            <input class="form-control" type="text" name="name" id="name" value="{{ old('name', $bankingServiceProvider->name) }}" required>
                            @if($errors->has('name'))
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="bank_type_id" id="bank_type_id" required>
                                @foreach($bank_types as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('bank_type_id') ? old('bank_type_id') : $bankingServiceProvider->bank_type->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="text" name="head_office" id="head_office" value="{{ old('head_office', $bankingServiceProvider->head_office) }}">
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="text" name="known_as" id="known_as" value="{{ old('known_as', $bankingServiceProvider->known_as) }}" required>
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="text" name="swift_code" id="swift_code" value="{{ old('swift_code', $bankingServiceProvider->swift_code) }}">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" name="category" id="category">
                                ...
                                @foreach(App\Models\BankingServiceProvider::CATEGORY_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('category', $bankingServiceProvider->category) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="text" name="website" id="website" value="{{ old('website', $bankingServiceProvider->website) }}">
