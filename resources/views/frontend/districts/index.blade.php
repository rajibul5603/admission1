        ...
        <div class="col-md-12">
            @can('district_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.districts.create') }}">
                            ...
                            <tbody>
                                @foreach($districts as $key => $district)
                                    <tr data-entry-id="{{ $district->id }}">
                                        ...
                                        <td>
                                            @can('district_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.districts.show', $district->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('district_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.districts.edit', $district->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('district_delete')
                                                <form action="{{ route('frontend.districts.destroy', $district->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('district_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.districts.massDestroy') }}",
