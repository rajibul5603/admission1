    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.education.update", [$education->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group">
                            ...
                            <select class="form-control" name="degree" id="degree" required>
                                ...
                                @foreach(App\Models\Education::DEGREE_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('degree', $education->degree) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
