        ...
        <div class="col-md-12">
            @can('upazila_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.upazilas.create') }}">
                            ...
                            <tbody>
                                @foreach($upazilas as $key => $upazila)
                                    <tr data-entry-id="{{ $upazila->id }}">
                                        ...
                                        <td>
                                            @can('upazila_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.upazilas.show', $upazila->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('upazila_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.upazilas.edit', $upazila->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('upazila_delete')
                                                <form action="{{ route('frontend.upazilas.destroy', $upazila->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('upazila_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.upazilas.massDestroy') }}",
