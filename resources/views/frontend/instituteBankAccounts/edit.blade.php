        ...
        <div class="col-md-12">
            ...
            <div class="card">
                <div class="card-header">
                    {{ trans('global.edit') }} {{ trans('cruds.instituteBankAccount.title_singular') }}
                </div>

                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.institute-bank-accounts.update", [$instituteBankAccount->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="banking_type_id" id="banking_type_id">
                                @foreach($banking_types as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('banking_type_id') ? old('banking_type_id') : $instituteBankAccount->banking_type->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="text" name="account_name" id="account_name" value="{{ old('account_name', $instituteBankAccount->account_name) }}" required>
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="number" name="account_number" id="account_number" value="{{ old('account_number', $instituteBankAccount->account_number) }}" step="1" required>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="bank_name_id" id="bank_name_id">
                                @foreach($bank_names as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('bank_name_id') ? old('bank_name_id') : $instituteBankAccount->bank_name->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="branch_name_id" id="branch_name_id">
                                @foreach($branch_names as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('branch_name_id') ? old('branch_name_id') : $instituteBankAccount->branch_name->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="routing_no_id" id="routing_no_id">
                                @foreach($routing_nos as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('routing_no_id') ? old('routing_no_id') : $instituteBankAccount->routing_no->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="eiin_id" id="eiin_id">
                                @foreach($eiins as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('eiin_id') ? old('eiin_id') : $instituteBankAccount->eiin->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('eiin'))
                                <div class="invalid-feedback">
