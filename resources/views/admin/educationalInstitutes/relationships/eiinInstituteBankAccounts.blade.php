            ...
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    ...
                    <div class="table-responsive">
                        <table class=" table table-bordered table-striped table-hover datatable datatable-eiinInstituteBankAccounts">
                            <thead>
                                <tr>
                                    <th width="10">
                                    ...
                                    <tr data-entry-id="{{ $instituteBankAccount->id }}">
                                        ...
                                        <td>
                                            @can('institute_bank_account_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('admin.institute-bank-accounts.show', $instituteBankAccount->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('institute_bank_account_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('admin.institute-bank-accounts.edit', $instituteBankAccount->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('institute_bank_account_delete')
                                                <form action="{{ route('admin.institute-bank-accounts.destroy', $instituteBankAccount->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@endcan
  ...
  let table = $('.datatable-eiinInstituteBankAccounts:not(.ajaxTable)').DataTable({ buttons: dtButtons })
